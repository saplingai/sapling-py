...
class SaplingClient:
    ...
    def aidetect(
        ...
        :param text: Text to score for AI-generated content (up to 200,000
            characters). Pass a list or tuple of 1-10 strings to score a batch
            in one request (sent as the API's ``texts`` parameter; the same
            options apply to every item, and the combined length is capped at
            200,000 characters). A batch response is ``{'results': [...]}``
